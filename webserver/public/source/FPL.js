...
$(function () {
  $(".fpl-form :nth-child(n)").keyup(function () {
    //convert all input elements to uppercase, no need to press caps lock
    ...
    if (
      ...
      //because some input texts might be empty we read only those which are not empty and not button 'Submit FPL'
...
function updateDepartureAerodromeField() {
  if (
    fpl.departureAerodrome.value.length > 0 &&
    fpl.departureAerodrome.value.length < 4
  ) {
    ...
    fetch(
      "http://localhost:3000/aerodrome?q=" + fpl.departureAerodrome.value
    ).then((response) => {
      response.json().then((data) => {
        if (data.status == "notfound") {
          ...
          let pointCoordinates = new ol.proj.fromLonLat([
            parseCoords[0],
            parseCoords[1],
          ]);
...
function updateDestinationAerodromeField() {
  if (
    fpl.destinationAerodrome.value.length > 0 &&
    fpl.destinationAerodrome.value.length < 4
  ) {
    ...
    fetch(
      "http://localhost:3000/aerodrome?q=" + fpl.destinationAerodrome.value
    ).then((response) => {
      response.json().then((data) => {
        if (data.status == "notfound") {
          $("#destinationAerodrome").css("background-color", "red");
...
$("#alternateAerodrome").change(function () {
  if (
    fpl.alternateAerodrome.value.length > 0 &&
    fpl.alternateAerodrome.value.length < 4
  ) {
    $("#alternateAerodrome").css("background-color", "red");
  } else {
    fetch(
      "http://localhost:3000/aerodrome?q=" + fpl.alternateAerodrome.value
    ).then((response) => {
      response.json().then((data) => {
...
$("#secondAlternateAerodrome").change(function () {
  if (
    fpl.secondAlternateAerodrome.value.length > 0 &&
    fpl.secondAlternateAerodrome.value.length < 4
  ) {
    $("#secondAlternateAerodrome").css("background-color", "red");
  } else {
    fetch(
      "http://localhost:3000/aerodrome?q=" + fpl.secondAlternateAerodrome.value
    ).then((response) => {
      response.json().then((data) => {
        if (
...
function updateRouteField() {
  ...
  function getRouteSegment(param, i, callback) {
    fetch("http://localhost:3000/coordinates?q=" + routeArray[i]).then(
      (response) => {
        response.json().then((data) => {
          if (data.status == "notfound") {
            if (param != undefined) {
            ...
            let parseCoords = JSON.parse(data.results).coordinates;
            fplRouteCoordinates[i] = new ol.proj.fromLonLat([
              parseCoords[0],
              parseCoords[1],
            ]);
          }
...
function updatePath() {
  ...
  function getAirwayName(param, i, callback) {
    fetch(
      "http://localhost:3000/airway?q1=" +
        fplPathValidNames[i] +
        "&q2=" +
        fplPathValidNames[i + 1]
    ).then((response) => {
      response.json().then((data) => {
        var airwayName;
        let airway;
  ...
  setTimeout(function () {
    ...
    for (var i = 0; i < fplAirways.length; i++) {
      ...
      let rotation = 0;
      let rot =
        (fplAirways[i].secondPoint[1] - fplAirways[i].firstPoint[1]) /
        (fplAirways[i].secondPoint[0] - fplAirways[i].firstPoint[0]);
    ...
    for (var i = 0; i < fplAirways.length; i++) {
      ...
      fplFeature = new ol.Feature({
        geometry: new ol.geom.LineString([
          fplAirways[i].firstPoint,
          fplAirways[i].secondPoint,
        ]),
      });
  ...
  fetch("http://localhost:3000/FPL?q=" + flightPlanArray).then((response) => {
    response.json().then((data) => {
      if (data.results == "ok") {
        ...
        setTimeout(function () {
          $("#submitFPL").css("color", "yellow");
        }, 4000);
        clearFPL();
        fetch("http://localhost:3000/Flights?q=" + flightArray).then(
          (response) => {
            response.json().then((data) => {});
          }
        );
  ...
  fetch("http://localhost:3000/GetMyFlightPlansContent").then((response) => {
    response.json().then((data) => {
      ...
      for (var i = 0; i < data.results.length; i++) {
        document.getElementById("load-fpl-content").innerHTML +=
          '<h6 style="margin-left:10px" id="savedFPL' +
          i +
          '">' +
          data.results[i] +
          "</h6>";
      }
      document.getElementById("save-fpl-content").innerHTML =
        document.getElementById("load-fpl-content").innerHTML;
    });
  });
};
...
$("#saveFPL").click(function () {
  ...
  fetch(
    "http://localhost:3000/WriteFPL?q1=" + filename + "&q2=" + fplToBeSaved
  ).then((response) => {
...
parseFPL = () => {
  ...
  if (fileFPLArray[9] != undefined) {
    ...
    if (positionS != -1) {
      ...
      if (tempSearch != -1 && FPL[19][tempSearch - 1] != " ") {
        //we make sure that J doesn't belong to J/ segment
