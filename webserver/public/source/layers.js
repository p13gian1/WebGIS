var styleJson1 =
  "https://api.maptiler.com/maps/e31d675e-d790-422f-9bdb-8e53ed2c77a3/style.json?key=uvpLhVXDl2QlWUYsQzXs";
var styleJson2 =
  "https://api.maptiler.com/maps/52cf67a2-9470-46ef-98b0-215feb1fe7be/style.json?key=uvpLhVXDl2QlWUYsQzXs";
...
function refreshCircleAerodrome() {
  ...
  if (aerodromeCircleLayer.getSource().getState() == "ready") {
    ...
    for (
      var i = 0;
      i < 60 / (5 * spaceOfCircle[aerodromeCircleState]) + 1;
      i++
    ) {
      aerodromeCircleFeature[i] = new ol.Feature({
        geometry: new ol.geom.Circle(
          ol.proj.fromLonLat([
            onWatchAerodromeLongitude,
            onWatchAerodromeLatitude,
          ]),
          11939 * spaceOfCircle[aerodromeCircleState] * i
        ), //0.0834 in WS84 is equal to 5 minutes or 5NM
      });
      aerodromeCircleLayer.getSource().addFeature(aerodromeCircleFeature[i]);
    }
    aerodromeCircleLayer
      .getSource()
      .getFeatures()
      [numberOfCircles].setStyle(
        new ol.style.Style({
